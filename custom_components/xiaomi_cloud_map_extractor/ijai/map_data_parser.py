...
class MapDataParserIjai(MapDataParser):
    ...
    def parse(raw: bytes, colors: Colors, drawables: Drawables, texts: Texts, sizes: Sizes,
        ...
        id1 = buf.get_uint8('id1')
        magic1 = buf.get_uint16('magic1')
        ...
        if map_data.image is not None and not map_data.image.is_empty:
            ...
            if len(map_data.rooms) > 0 and map_data.vacuum_position is not None:
                map_data.vacuum_room = MapDataParserIjai.get_current_vacuum_room(buf, map_data.vacuum_position)
    ...
    def parse_image(buf: ParsingBuffer, colors: Colors, image_config: ImageConfig, draw_cleaned_area: bool) \
        ...
        for number, room in rooms_raw.items():
            rooms[number] = Room(number, MapDataParserIjai.image_to_map(room[0] + image_left),
        ...
        return ImageData(image_size, image_top, image_left, image_height, image_width, image_config,
                         image, MapDataParserIjai.map_to_image,
    ...
    def parse_history(buf: ParsingBuffer) -> Path:
        ...
        for _ in range(history_count):
            ...
            path_points.append(MapDataParserIjai.parse_position(buf, 'path'))
    ...
    def parse_restricted_areas(buf: ParsingBuffer) -> Tuple[List[Wall], List[Area]]:
        ...
        for _ in range(area_count):
            ...
            p1 = MapDataParserIjai.parse_position(buf, 'p1')
            p2 = MapDataParserIjai.parse_position(buf, 'p2')
            p3 = MapDataParserIjai.parse_position(buf, 'p3')
            p4 = MapDataParserIjai.parse_position(buf, 'p4')
    ...
    def parse_rooms(buf: ParsingBuffer, map_data_rooms: Dict[int, Room]):
        ...
        for _ in range(room_count):
            ...
            room_text_pos = MapDataParserIjai.parse_position(buf, 'room.text_pos')
    ...
    def parse_position(buf: ParsingBuffer, name: str, with_angle: bool = False) -> Optional[Point]:
        ...
        if x == MapDataParserIjai.POSITION_UNKNOWN or y == MapDataParserIjai.POSITION_UNKNOWN:
            return None
        a = None
        if with_angle:
