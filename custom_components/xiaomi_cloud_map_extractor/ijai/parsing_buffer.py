...
class ParsingBuffer:
    ...
    def get_uint16_remove_parity(self, field: str) -> int:
        if self._length < 2:
            raise ValueError(f"error parsing {self._name}.{field} at offset {self._offs:#x}: buffer underrun")
        lo = self._data[self._offs] 
        hi = self._data[self._offs + 1]
        self._offs += 2
        self._length -= 2
        return ((hi^1) << 7) ^ lo
    ...
    def get_bytes(self, count:int, field:str) -> bytes:
        if self._length < count:
            raise ValueError(f"error parsing {self._name}.{field} at offset {self._offs:#x}: buffer underrun")
        self._offs += count
        self._length -= count
        return self._data[self._offs - count:self._offs].decode('utf-8')

    def get_string_len8(self, field: str) -> str:
        n = self.get_uint8(field + '.len')
        return get_bytes(n, field)
