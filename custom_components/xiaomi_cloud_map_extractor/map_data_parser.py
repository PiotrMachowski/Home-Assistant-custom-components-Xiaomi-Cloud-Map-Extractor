...
class ViomiMapDataParser:
    ...
    def parse(raw: bytes, colors, drawables, texts, sizes, image_config):
        ...
        featureFlags = buf.get_uint32('featureFlags')
        map_id = buf.peek_uint32('map_id')
        _LOGGER.debug('featureFlags: 0x%x, map_id: %d', featureFlags,  map_id)

        if featureFlags & 0x00000001 != 0:
            ViomiMapDataParser.parse_section(buf, 'robot_status', map_id)
            buf.skip('unknown1', 0x28)

        if featureFlags & 0x00000002 != 0:
            ViomiMapDataParser.parse_section(buf, 'image', map_id)
            map_data.image, map_data.rooms = ViomiMapDataParser.parse_image(buf, colors, image_config)

        if featureFlags & 0x00000004 != 0:
            ViomiMapDataParser.parse_section(buf, 'history', map_id)
            map_data.path = ViomiMapDataParser.parse_history(buf)

        if featureFlags & 0x00000008 != 0:
            ViomiMapDataParser.parse_section(buf, 'charge_station', map_id)
            map_data.charger = ViomiMapDataParser.parse_position(buf, 'pos')
            foo = buf.get_float32('foo')
            _LOGGER.debug('pos: %s, foo: %f', map_data.charger, foo)

        if featureFlags & 0x00000010 != 0:
            ViomiMapDataParser.parse_section(buf, 'restricted_areas', map_id)
            map_data.walls, map_data.no_go_areas = ViomiMapDataParser.parse_restricted_areas(buf)

        if featureFlags & 0x00000020 != 0:
            ViomiMapDataParser.parse_section(buf, 'cleaning_areas', map_id)
            map_data.zones = ViomiMapDataParser.parse_cleaning_areas(buf)

        if featureFlags & 0x00000040 != 0:
            ViomiMapDataParser.parse_section(buf, 'navigate', map_id)
            buf.skip('unknown1', 4)
            map_data.goto = ViomiMapDataParser.parse_position(buf, 'pos')
            foo = buf.get_float32('foo')
            _LOGGER.debug('pos: %s, foo: %f', map_data.goto, foo)

        if featureFlags & 0x00000080 != 0:
            ViomiMapDataParser.parse_section(buf, 'realtime', map_id)
            buf.skip('unknown1', 5)
            map_data.vacuum_position = ViomiMapDataParser.parse_position(buf, 'pos')
            foo = buf.get_float32('foo')
            _LOGGER.debug('pos: %s, foo: %f', map_data.vacuum_position, foo)

        if featureFlags & 0x00000800 != 0:
            ViomiMapDataParser.parse_section(buf, 'unknown1', map_id)
            buf.skip('unknown1', 4)

        if featureFlags & 0x00001000 != 0:
            ViomiMapDataParser.parse_section(buf, 'rooms', map_id)
            ViomiMapDataParser.parse_rooms(buf)

        if featureFlags & 0x00002000 != 0:
            ViomiMapDataParser.parse_section(buf, 'unknown2', map_id)
            ViomiMapDataParser.parse_unknown_section(buf)

        if featureFlags & 0x00004000 != 0:
            ViomiMapDataParser.parse_section(buf, 'unknown3', map_id)
            ViomiMapDataParser.parse_unknown_section(buf)
...
#                map_data.vacuum_room = MapDataParser.get_current_vacuum_room(img_start, raw, map_data.vacuum_position)
    ...
    def parse_image(buf, colors, image_config):
        ...
        if image_height \
            ...
            image_config[CONF_TRIM][CONF_TOP] = 0
            image_config[CONF_TRIM][CONF_BOTTOM] = 0
        image, rooms = ImageHandler.parse_viomi(buf, image_width, image_height, colors, image_config)
    ...
    def parse_history(buf):
        ...
        for _ in range(history_count):
            ...
            path_points.append(ViomiMapDataParser.parse_position(buf, 'path'))
    ...
    def parse_restricted_areas(buf):
        ...
        for _ in range(area_count):
            ...
            p1 = ViomiMapDataParser.parse_position(buf, 'p1')
            p2 = ViomiMapDataParser.parse_position(buf, 'p2')
            p3 = ViomiMapDataParser.parse_position(buf, 'p3')
            p4 = ViomiMapDataParser.parse_position(buf, 'p4')
    ...
    def parse_rooms(buf):
        ...
        for _ in range(room_count):
            ...
            room_text_pos = ViomiMapDataParser.parse_position(buf, 'xxx')
    ...
    def parse_section(buf, name, map_id):
        ...
        if magic != map_id:
            raise ValueError(f"error parsing section {name} at offset {buf._offs - 4:#x}: magic check failed {magic:#x}") #FIXME

    @staticmethod
    def parse_position(buf, name):
        x = buf.get_float32(name + '.x')
        y = buf.get_float32(name + '.y')
        if x == ViomiMapDataParser.POSITION_UNKNOWN or y == ViomiMapDataParser.POSITION_UNKNOWN:
            return None
